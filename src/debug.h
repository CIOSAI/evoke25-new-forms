...
template<size_t N>
static bool __forceinline dbg_nocheck_shader_compilation(int (&programs)[N]){
	 #if OPENGL_DEBUG
...
template<size_t N>
static bool __forceinline dbg_check_shader_compilation(int (&programs)[N]){
	#if OPENGL_DEBUG
		size_t arrayLength = N;
		for(int i = 0; i < arrayLength; i++){
			int pid = programs[i];
	...
	const void* userParam)
{
	...
	if (
		...
		|| severity == 33387 
		|| severity == 37191
		|| (type == 33356 && severity == 37190)
	) // 37191 is buffer leaking to cpu mem warning.
		return;
